...
def calculate_payoff_from_fight(bet_amount:float, fight:int, fighter:str, fee:float=.05)->float:
    """"
    ...
    # get the fight
    fight_data = supabase_client.table("current_odds").select("*").eq("event_id", fight).order('timestamp', desc=True).limit(1).execute().data[0]
...
def create_bet_request(bet_amount:float, fight:int, fighter:str, venmo_id:str, timestamp=int(time.time()), paid:bool=False, bet_bounds:tuple[float]=(0.1, 10.0)): 
    """
    ...
    if bet_amount < bet_bounds[0] or bet_amount > bet_bounds[1]:
        raise ValueError(f"Bet amount must be between {bet_bounds[0]} and {bet_bounds[1]}")
    ...
    user = venmo_client.user.search_for_users(query=venmo_id, username=True)[0] # assume the first person whos ID matches is correct
    user_id = user.id
    request = venmo_client.payment.request_money(amount=bet_amount, target_user_id=user_id, note=f"Fight: {fight}; fighter: {fighter}; amount: {bet_amount}; timestamp: {timestamp}; potential_payout: {payout}")

    # get the pending transaction ID
    pending_txs = venmo_client.payment.get_charge_payments()
    for pending in pending_txs: 
        if pending.target.id == user_id and pending.note==f"Fight: {fight}; fighter: {fighter}; amount: {bet_amount}; timestamp: {timestamp}; potential_payout: {payout}":
...
def check_paid_bet_requests():
    ...
    pending_txns_db = supabase_client.table('pending_txns').select('*').eq('paid', False).eq('cancelled', False).execute().data
    ...
    for db_txn in pending_txns_db:
        ...
        if paid:
            ...
            supabase_client.table('pending_txns').update({'paid': True}).eq('payment_id', db_txn['payment_id']).execute()
            db_txn['paid'] = True
...
def pay_bets_out(fight:int, victorious_fighter:int):
    ...
    completed_txns = supabase_client.table('completed_txns').select('*').eq('event', fight).eq('outcome', victorious_fighter).eq('paid', True).eq("cancelled", False).execute().data
    print(completed_txns)

    for txn in completed_txns:
        print(completed_txns)
        # turn the transaction into "cancelled"
        supabase_client.table('completed_txns').update({'cancelled': True}).eq('payment_id', txn['payment_id']).execute()
        # pay out the bet
        venmo_client.payment.send_money(amount=txn['payout'], 
                                        target_user_id=txn['venmo_id'], 
                                        note=f"Congratulations! You won on fight {fight} on fighter {victorious_fighter}")
        # update the DB to reflect that the transaction has been paid
        supabase_client.table('completed_txns').update({'paid': True}).eq('payment_id', txn['payment_id']).execute()
